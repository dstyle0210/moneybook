    ...
    try{
        ...
        new Promise((res)=>{
            ...
            firebase.database().ref(getReceiptsUrl()).on("value", (snapshot) => {
                ...
                ReactDOM.render( <S_receiptsBook receipts={useReceipts} user={user} /> ,$("#receiptsBook").get(0));
            });
        });
    }catch(e){
        alert(e);
...
const S_receiptsBook = ({receipts,user}) =>{
    return(
    <React.Fragment>
        <S_receiptsList _receipts={receipts} user={user} />
        <S_nowMonthTotal receipts={receipts} user={user} />
    </React.Fragment>
    );
};